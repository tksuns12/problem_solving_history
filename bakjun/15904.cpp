...
int main()
{
    ...
    char temp[1002];
    string after = "";
    cin.getline(temp, 1002);
    string s = string(temp);
    vector<char> table = {'U', 'C', 'P', 'C'};

    for (char c : s)
    {
        if (isupper(c) && c == *table.begin())
        {
            after += c;
...
/*문자열을 받으려면 cin.getline을 써야 하고 string을 바로 못 쓰고 char배열을 만들어서 받은 다음에 string으로 변환해줘야 한다.*/